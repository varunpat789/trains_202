...
automatic
	...
	CMP r4, #1                        ;if ignore2, don't carry out train movement
    BLLT green_led                    ; turn on green led to indicate movement, keep on
	
	BL long_delay
	
	CMP r4, #1							;compare again, don't carry out train movement if ignore2/1 high
...
train_motor
	...
	PUSH {LR,r4,r0,r2,r5,r8,r10,r11} 	            ;want to listen on r9(direction)
	...
	MOV r5, #0 							; Current rotation state (1 = accel, 2 = steady, 3 = deccel)
...
green_led
	...
	PUSH {r3,r4,r6,r7}  ;push everything except r8(status)
	LDR r6, =EXTI_BASE
	LDR r7, [r6, #EXTI_IMR1]; mask to toggle IRQ handler
	EOR r7, #0x2000; 
	STR r7, [r6, #EXTI_IMR1]
	...
	POP {r3,r4,r6,r7}   ;pop back registers
...
flag2_check1 
	...
	MOVEQ r7, #1  ; if so, set interrupt flag2 to high 
	BX LR         ;return back
